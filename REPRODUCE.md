...
|---|---|---|---|
| FCF upstream → `models/core/fcf/upstream/` | `github.com/f-c-forgetting/FCF` @ `e65e96a4` | **none** → fetch, not vendored | faithful FCF-P/E training |
...
python models/core/fcf/legacy_reimpl/train.py  --config models/core/fcf/legacy_reimpl/configs/nudity_fcf_p.yaml          # FCF-P reimpl (~52)
python models/core/fcf/legacy_reimpl/train.py  --config models/core/fcf/legacy_reimpl/configs/nudity_fcf_e.yaml          # FCF-E reimpl
python models/core/lsse/train_lsse.py --config models/core/lsse/configs/nudity_lsse.yaml     # LSSE (+ --use_plu etc.)
python models/core/dace/train_dace.py --config models/core/dace/configs/nudity_dace.yaml     # DACE (negative result)
python models/comparison/novel/train.py --config models/comparison/novel/configs/nudity_v2.yaml --manifold spherical

# --- UNet tracks ---
python models/comparison/odace/train_odace.py --config models/comparison/odace/configs/nudity_odace.yaml      # ODACE v3 (champion)
python models/comparison/odace/train_odace.py --config models/comparison/odace/configs/nudity_odace_v15.yaml  # ODACE v1.5
python models/comparison/esd/train_esd.py --config models/comparison/esd/configs/nudity_esd_u.yaml   # ESD-u

# --- FAITHFUL FCF (authors' code; uses only torch+transformers+pandas, no LDM) ---
cd models/core/fcf/upstream
...
#   load CLIPTextModel, load_state_dict(<trained>.pt), .save_pretrained("models/core/fcf/official_fcf_p/final")
...
|---|---|
| `odace_v3` / `odace_v15` | `models/comparison/odace/outputs/odace_v3/final` · `models/comparison/odace/outputs/odace_v15/final` (UNet) |
| `esd_u` | `models/comparison/esd/outputs/esd_u/final` (UNet) |
| `fcf_p` / `fcf_e` (reimpl) | `fcf/outputs/fcf_{p,e}_nudity/final` (CLIPTextModel) |
| `fcf_p_official` / `fcf_e_official` | `models/core/fcf/official_fcf_{p,e}/final` (CLIPTextModel) |
...
python models/core/fcf/eval_fullset.py            # raw + FCF-P/E full-set (1 img/prompt)
python models/core/fcf/rescore_fullset_paperrule.py   # paper presence-rule headline
python models/core/fcf/eval_fullset_all.py        # full-set for all Table-A methods
python models/core/fcf/eval_coco_fid5k.py         # COCO FID-5K (paper ~15 scale)
python models/core/fcf/eval_violence_q16.py       # violence Q16 locality (all models)
```
...
- **Run the documented commands above, not the `*.sh` wrappers.** Several `*/experiments/*.sh`,
  `models/comparison/run_eval.sh`, `eval/run_BC.sh`, and `models/comparison/novel/scripts/dispatch/*.sh` contain
...
- **Q16 under transformers 5.9:** `models/core/lsse/evaluation/q16_classifier.py` breaks (returns a ModelOutput);
  `models/core/fcf/eval_violence_q16.py` ships a self-contained, version-robust Q16 scorer.
- **FCF violence data exists** (`models/core/fcf/upstream/data/train/violence.csv`) if you want to
