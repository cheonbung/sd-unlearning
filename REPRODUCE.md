...
|---|---|---|---|
| FCF upstream → `compare/fcf_repro/upstream/` | `github.com/f-c-forgetting/FCF` @ `e65e96a4` | **none** → fetch, not vendored | faithful FCF-P/E training |
...
| SD 1.4/1.5/2.1, CLIP, Safe-CLIP, NudeNet | Hugging Face | per model | auto-downloaded; gated SD2.1/SD1.5 fall back to mirrors in `xmodel/xeval.py:REGISTRY` |
...
python fcf/train.py  --config fcf/configs/nudity_fcf_p.yaml          # FCF-P reimpl (~52)
python fcf/train.py  --config fcf/configs/nudity_fcf_e.yaml          # FCF-E reimpl
python lsse/train_lsse.py --config lsse/configs/nudity_lsse.yaml     # LSSE (+ --use_plu etc.)
python dace/train_dace.py --config dace/configs/nudity_dace.yaml     # DACE (negative result)
python fcf-novel-methods/train.py --config fcf-novel-methods/configs/nudity_v2.yaml --manifold spherical

# --- UNet tracks ---
python odace/train_odace.py --config odace/configs/nudity_odace.yaml      # ODACE v3 (champion)
python odace/train_odace.py --config odace/configs/nudity_odace_v15.yaml  # ODACE v1.5
python baselines/esd/train_esd.py --config baselines/esd/configs/nudity_esd_u.yaml   # ESD-u

# --- FAITHFUL FCF (authors' code; uses only torch+transformers+pandas, no LDM) ---
cd compare/fcf_repro/upstream
...
#   load CLIPTextModel, load_state_dict(<trained>.pt), .save_pretrained("compare/fcf_repro/official_fcf_p/final")
...
`xmodel/xeval.py:REGISTRY` maps each model key to where its checkpoint must live. After training,
ensure the trained model sits at the registered path, e.g.:

| Key | Expected path (REGISTRY) |
|---|---|
| `odace_v3` / `odace_v15` | `odace/outputs/odace_v3/final` · `odace/outputs/odace_v15/final` (UNet) |
| `esd_u` | `baselines/esd/outputs/esd_u/final` (UNet) |
| `fcf_p` / `fcf_e` (reimpl) | `fcf/outputs/fcf_{p,e}_nudity/final` (CLIPTextModel) |
| `fcf_p_official` / `fcf_e_official` | `compare/fcf_repro/official_fcf_{p,e}/final` (CLIPTextModel) |
...
python xmodel/xeval.py     --models raw_v14,odace_v3,esd_u,fcf_p_official,fcf_e_official,sld_medium,safeclip
python xmodel/eval_coco.py --models raw_v14,odace_v3,fcf_p_official        # COCO-300 FID/CLIP/LPIPS/IQ

# Paper-aligned FCF reproduction (full prompt sets, 4-label):
python compare/fcf_repro/eval_fullset.py            # raw + FCF-P/E full-set (1 img/prompt)
python compare/fcf_repro/rescore_fullset_paperrule.py   # paper presence-rule headline
python compare/fcf_repro/eval_fullset_all.py        # full-set for all Table-A methods
python compare/fcf_repro/eval_coco_fid5k.py         # COCO FID-5K (paper ~15 scale)
python compare/fcf_repro/eval_violence_q16.py       # violence Q16 locality (all models)
```
...
- **Run the documented commands above, not the `*.sh` wrappers.** Several `*/experiments/*.sh`,
  `baselines/run_eval.sh`, `xmodel/run_BC.sh`, and `fcf-novel-methods/scripts/dispatch/*.sh` contain
...
- **HF model drift:** `runwayml/stable-diffusion-v1-5` was removed and SD2.1-base is gated; mirrors
  are wired into `xmodel/xeval.py:REGISTRY` and fall back automatically.
- **Q16 under transformers 5.9:** `lsse/evaluation/q16_classifier.py` breaks (returns a ModelOutput);
  `compare/fcf_repro/eval_violence_q16.py` ships a self-contained, version-robust Q16 scorer.
- **FCF violence data exists** (`compare/fcf_repro/upstream/data/train/violence.csv`) if you want to
