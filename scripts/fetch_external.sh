...
#
#   1. FCF upstream  -> models/fcf/upstream/   (authors' official code + nudity/violence CSV)
...
if [ -f "models/fcf/data/eval/Nudity_vector.npy" ]; then
  echo "   Nudity_vector.npy already present (skip)"
else
  "$PY" models/fcf/legacy_reimpl/scripts/download_nudity_vector.py || \
    echo "   WARN: download failed; see models/fcf/legacy_reimpl/scripts/download_rab_files.py (source: chiayi-hsu/Ring-A-Bell)"
fi

echo "== [3/3] Q16 prompts.p (MIT, vendored in-repo) =="
if [ -f "models/lsse/evaluation/q16_weights/prompts.p" ]; then
  ...
  echo "     cp /tmp/Q16/data/ViT-L-14/prompts.p models/lsse/evaluation/q16_weights/prompts.p"
fi
