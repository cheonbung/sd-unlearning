...
eval/eval_coco.py uses N_GEN=300 / N_REAL=600. That tiny sample size is exactly why its FID sits
...
Data: eval/data/coco/captions_val2017.json (already present) -> captions{N}.txt + real{N}/ images.
Gen -> eval/outputs/<model>_coco5k{tag}/ (gitignored). Result -> models/core/fcf/coco5k{tag}.json.

Run (WSL conda env lsse):
  python models/core/fcf/eval_coco_fid5k.py --prep_only                  # download 5000 real imgs
  python models/core/fcf/eval_coco_fid5k.py --n 24 --tag _smoke          # smoke test
  python models/core/fcf/eval_coco_fid5k.py                              # full FID-5K (P/E/raw)
"""
...
def main():
    ...
    for model in models:
        ...
        try:
            ...
            out_dir = REPO / "eval" / "outputs" / f"{model}_coco5k{args.tag}"
            ...
            ref = REPO / "eval" / "outputs" / f"raw_v14_coco5k{args.tag}"
