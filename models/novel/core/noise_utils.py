"""Noise text generation utilities for FCF (paper appendix spec).

The FCF paper defines P_noise as: replace the explicit target concept in
P_explicit with random textual noise. The noise must:
...
This file is a verbatim copy of fcf/noise_utils.py — kept local so that this
sub-project has zero runtime dependency on the parent thesis package.
"""
...
def generate_random_noise_text(length: int = 5, seed: int = None) -> str:
    ...
    Returns:
        A noise string with no repeated characters, containing at least one
        symbol, one letter, and one digit.
    """
