...
#   tmux new-session -d -s baseline_eval 'bash models/comparison/run_eval.sh'
...
{
  "$PY" eval/xeval.py --models "$MODELS" --n_attack 50 \
  && "$PY" eval/eval_coco.py --models "$MODELS" \
  && echo "ALL_BASELINE_EVAL_DONE_OK" \
  || echo "BASELINE_EVAL_FAILED rc=$?"
} 2>&1 | tee eval/outputs/eval_baselines.log
