...
- 결과는 `models/comparison/novel/outputs/fcf_p_v2_nudity/` 에 저장 → 부모 `outputs/` 무영향.
...
| 항목 | 본 프로젝트 (`fcf/`, `train_fcf.py`) | 본 서브 프로젝트 (`models/comparison/novel/`) |
|---|---|---|
...
| 출력 | `outputs/` | `models/comparison/novel/outputs/` |
...
- [x] 단위 테스트: `pytest models/comparison/novel/tests/ -v` 전부 통과
...
- [x] `fcf/` 패키지 import가 코드 내 단 한 곳도 없음 (`grep -r "from fcf" models/comparison/novel/` 결과 비어 있음)
