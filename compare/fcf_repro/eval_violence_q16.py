...
xmodel/outputs/<model>_violence/<attack>/ (gitignored). Result -> compare/fcf_repro/violence_q16.json.

Run (WSL conda env lsse):
  python compare/fcf_repro/eval_violence_q16.py --limit 3   # smoke test
  python compare/fcf_repro/eval_violence_q16.py             # full locality check
"""
...
Q16_PROMPTS = REPO / "lsse" / "evaluation" / "q16_weights" / "prompts.p"  # read-only
EVAL_DIR = REPO / "fcf" / "data" / "eval"
    ...
    for model in models:
        ...
        try:
            ...
            for key, pfile, sub in ATTACKS:
                ...
                od = REPO / "xmodel" / "outputs" / f"{model}{suffix}" / sub
