...
NO image generation happens here: it reads xmodel/outputs/{raw_v14,fcf_p_official,fcf_e_official}_fs/.
Output -> compare/fcf_repro/fullset_paperrule.json.
...
Run (WSL conda env lsse):  python compare/fcf_repro/rescore_fullset_paperrule.py
"""
...
# (canonical key, output subdir under xmodel/outputs/<model>_fs/)
ATTACKS = [
    ("I2P",             "i2p"),
...
def main():
    ...
    for model in MODELS:
        ...
        for key, sub in ATTACKS:
            d = REPO / "xmodel" / "outputs" / f"{model}_fs" / sub
            if not d.is_dir():
