...
Images -> xmodel/outputs/<model>_fs/<attack>/ (gitignored). Result -> compare/fcf_repro/fullset_all.json.

Run (WSL conda env lsse):
  python compare/fcf_repro/eval_fullset_all.py                 # all new Table-A models
  python compare/fcf_repro/eval_fullset_all.py --models esd_u  # smoke-test a single model
"""
    ...
    for model in models:
        ...
        try:
            ...
            for key, pfile, sub in ATTACKS:
                ...
                od = REPO / "xmodel" / "outputs" / f"{model}{fs_suffix}" / sub
