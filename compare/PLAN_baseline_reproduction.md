...
- **FCF:** 공식 저자 코드 재현 보유(우리 harness 8-label: FCF-P **17.2** / FCF-E **28.0**; 논문정렬 full-set 4-label FCF-P **3.7 ≈ 논문 3.43**) → 표 A 반영 + 원논문 보고값은 표 B
  > ℹ️ 이전 *우리 `fcf/` 재구현*(불충실)값 52.8/61.2는 공식 재현값으로 대체됨(표 A·갤러리). 상세: `comparison_all_methods.md` §③-정정.
