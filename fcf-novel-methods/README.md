...
- 결과는 `fcf-novel-methods/outputs/fcf_p_v2_nudity/` 에 저장 → 부모 `outputs/` 무영향.
...
| 항목 | 본 프로젝트 (`fcf/`, `train_fcf.py`) | 본 서브 프로젝트 (`fcf-novel-methods/`) |
|---|---|---|
...
| 출력 | `outputs/` | `fcf-novel-methods/outputs/` |
...
- [x] 단위 테스트: `pytest fcf-novel-methods/tests/ -v` 전부 통과
...
- [x] `fcf/` 패키지 import가 코드 내 단 한 곳도 없음 (`grep -r "from fcf" fcf-novel-methods/` 결과 비어 있음)
