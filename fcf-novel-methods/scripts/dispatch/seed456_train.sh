...
LOG="fcf-novel-methods/logs/train_seed456_sph_ot_${TS}.log"
...
if [ -d 'fcf-novel-methods/outputs/fcf_p_v2_nudity' ]; then
  echo "!!! fcf_p_v2_nudity already exists — aborting to prevent overwrite. Rename it first."
  exit 2
fi

python fcf-novel-methods/train.py   --config fcf-novel-methods/configs/nudity_v2.yaml   --seed 456   --manifold spherical   --ot_noise_file outputs/ot_noise/nudity_learned.json   2>&1 | tee "${LOG}"
